...
###   This script attempts to download the signature file SHA256SUMS.asc from shahbit.org
...
WORKINGDIR="/tmp/shahbit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://shahbit.org/bin/"
VERSIONPREFIX="shahbit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'shahbit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Shahbit Core binary release signing key installed?"
   fi
