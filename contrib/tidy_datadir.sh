...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Shahbit database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Shahbit datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Shahbit datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Shahbit 0.7 datadir."
    ;;
  3)
    echo "Detected Shahbit pre-0.8 datadir."
    ;;
  4)
    echo "Detected Shahbit 0.8 datadir."
    ;;
esac
